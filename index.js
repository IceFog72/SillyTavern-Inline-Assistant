...
function bindInput(id, key, type = 'value') {
    ...
    if (!element) return;
    const s = settings();
    if (type === 'checked') element.checked = Boolean(s[key]);
    else element.value = s[key];
    element.addEventListener(type === 'checked' ? 'change' : 'input', () => {
        s[key] = type === 'checked' ? element.checked : element.type === 'number' ? Number(element.value) : element.value;
        save();
        renderRuntime();
...
function updateLanguage(key, value) {
    ...
    if (matching) matching.value = value;
    save();
    scheduleWork('settings');
}
